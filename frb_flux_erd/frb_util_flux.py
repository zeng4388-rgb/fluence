...
class AstroDistribution:
    ...
    def log_distr_fdmwz(self, dnu, logflux, dme, logw, z, alpha, logfs, logf0, mu, sigma, gtype=None):
        """
        Logarithmic joint distribution function of flux, DM and redshift for FLUX FUNCTION
        """
        logint1 = self.log_IntBeam(logflux, alpha, logfs, logf0)
        #print logint1
    ...
    def log_distr_fdmw(self, dnu, logflux, dme, logw, alpha, logfs, logf0, mu, sigma, gtype=None):
        """
        ...
        :param dme: dme
        :param logfs: logarithmic cut-off flux
        :param alpha: lf index
        :param logf0: minimum of lf flux
        ...
        lik = 0
        for z in vz:
            likv = np.exp(self.log_distr_fdmwz(dnu, logflux, dme, logw, z, alpha, logfs, logf0, mu, sigma, gtype=gtype))
            lik += z * stepz * likv
        ind = lik > 0
        ind2 = lik <= 0
        loglik = lik.copy()
    ...
    def Norm1D(self, sn0, bw, npol, g, tsys, dnu, alpha, logfs, logf0, mu, sigma):
        """
        Normalization factor for dimensionless likelihood using FLUX
        """
        stepz = (np.log(self.Zmax) - np.log(self.Zmin)) / 1000.
        vz = np.exp(np.arange(np.log(self.Zmin), np.log(self.Zmax), stepz))
        stepeps = (1-0.5) / 200.
        ...
        for z in vz:
            ...
            ind = logft < logf0
            logft[ind] = logf0
            int_eps = np.zeros(logft.shape)
            for i in np.arange(len(logft)):
                int_eps[i] = np.sum(self.IntLum(veps, alpha, logfs, logft[i]*np.ones(veps.shape))/veps/np.log(2)*stepeps)
...
class EventRate:
    ...
    def rate_2d(self, sn0, bw, npol, g, tsys, dnu, phis, alpha, logfs, logf0, mu, sigma):
        ...
        for z in vz:
            ...
            logft[ind] = logf0
            int_eps = np.zeros(logft.shape)
            for i in range(len(logft)):
                int_eps[i] = np.sum(phis*self.ad.IntLum(veps, alpha, logfs, logft[i]*np.ones(veps.shape))/veps/np.log(2)*stepeps)
